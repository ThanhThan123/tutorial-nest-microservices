  ...
  if (mode === 'prune') {
    ...
    for (const doc of seedData) {
      ...
      if (!exists) {
        ...
        console.log(` Insert into  [${seed.collection}] -> `, doc);
      }
    }
...
async function bootstrap() {
  const dirPath = process.argv[2]; // ví dụ: apps/user-access/seeder(đường dẫn)
  const mode = process.argv[3] || 'migrate'; // prune | migrate

  if (!dirPath) {
    console.error(' Please provider a seeder direction path');
    process.exit(1);
  }
  ...
  try {
    ...
    console.log('All seeder excuted successfully !');
  } catch (err) {
