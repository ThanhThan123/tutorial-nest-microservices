...
export class SagaOrchestrationService {
  ...
  async execute<TContext extends SagaContext>(
    ...
    try {
      ...
      this.logger.log(`Saga ${sagaId} completed sucessfully`);
  ...
  private async executeStep<TContext extends SagaContext>(
    ...
    try {
      ...
      await this.repository.markStepComplete(sagaId, stepIndex, result.data);
  ...
  private async compensate<TContext extends SagaContext>(
    ...
    this.logger.log(`Starting Compensating for saga ${sagaId}`);
    ...
    for (let i = completedSteps.length - 1; i >= 0; i--) {
      ...
      if (step && step.compensate) {
        try {
          ...
          this.logger.log(`Step ${step.name} compensated step ${step.name} for saga ${sagaId}`);
        } catch (error) {
          const errorMessage = error instanceof Error ? error.message : 'Unknown error';
